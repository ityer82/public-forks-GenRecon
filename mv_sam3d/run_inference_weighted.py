...
def run_weighted_inference(
    ...
    # Reference-view world-to-camera extrinsic (post filename-matching against the DA3
    # npz, i.e. the actual view 0 used for this object's masked view list, not necessarily
    # the dataset's global frame 0). translation/rotation/scale above are expressed in this
    # view's camera space, so callers need this exact matrix (inverted) to place the mesh
    # in world space — see merge_glb_with_da3_aligned's use of da3_extrinsics[0].
    if da3_extrinsics is not None and len(da3_extrinsics) > 0:
        params['ref_extrinsic'] = np.asarray(da3_extrinsics[0])
