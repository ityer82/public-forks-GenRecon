...
try:
    ...
    # kaolin is only used here for a debug shape-assertion helper (all call
    # sites below pass throw=False and only check shape, not dtype/device),
    # so fall back to a minimal equivalent rather than requiring all of
    # kaolin just for this.
    def check_tensor(tensor, shape=None, throw=False):
        if shape is None:
            return True
        if tensor.dim() != len(shape):
            return False
        return all(expected is None or actual == expected for actual, expected in zip(tensor.shape, shape))
