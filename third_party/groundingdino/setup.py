...
def get_extensions():
    ...
    # Modern setuptools' egg_info/build_py require ext_modules `sources` to be
    # relative to this setup.py's own directory (an absolute path here raises
    # DistutilsSetupError) -- `sources` is already absolute at this point
    # (built from `extensions_dir`, itself derived from `os.path.abspath`), so
    # convert back to relative instead of the redundant (and here harmful)
    # os.path.join(extensions_dir, s) upstream used, which is a no-op on an
    # already-absolute `s`.
...
if __name__ == "__main__":
    ...
    # NOTE: plain metadata (name/version/author/url/description/license) is
    # intentionally NOT passed here -- it's owned statically by this package's
    # own pyproject.toml [project] table instead, to avoid setuptools' legacy-
    # vs-PEP621 metadata reconciliation (which errors on a few of these fields
    # as vendored here, e.g. license being a raw LICENSE-file string rather
    # than a PEP 639 table).
    setup(
