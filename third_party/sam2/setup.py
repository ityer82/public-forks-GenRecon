...
REQUIRED_PACKAGES = [
    ...
    "tqdm>=4.66.1",
    "hydra-core>=1.3.2",
    "iopath>=0.1.9",  # relaxed from upstream's >=0.1.10: only 0.1.9 is available on the pinned cu128 index
    "pillow>=9.4.0",
]
...
# Setup configuration
# NOTE: plain metadata (name/version/description/url/author/license/readme) is
# intentionally NOT passed here -- it's owned statically by this package's own
# pyproject.toml [project] table instead, to avoid setuptools' legacy-vs-PEP621
# metadata reconciliation (which errors on a few of these fields as vendored
# here, e.g. license being a raw LICENSE-file string rather than a PEP 639
# table). Only genuinely build-time-only config (packages/extensions) remains.
setup(
